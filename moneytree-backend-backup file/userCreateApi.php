...
if(isset($data['name']) and isset($data['email']) and isset($data['phone'])and isset($data['passwordofuser']) ){
$name = $data['name'];
$email = $data['email'];
$phone = $data['phone'];
$passwordof = $data['passwordofuser'];
...
if(mysqli_num_rows($result1)==0){
    ...
    if(mysqli_num_rows($result2)==0){
        $sql3 = "INSERT INTO `users`(`name`, `email`, `phone_number`, `token`,`referal_id`,`password`) VALUES ('{$name}','{$email}','{$phone}','{$token}','{$referal}','{$passwordof}')";
